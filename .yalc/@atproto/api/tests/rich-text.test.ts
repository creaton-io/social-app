...
describe('RichText#insert', () => {
  const input = new RichText({
    text: 'hello world',
    facets: [{index: {byteStart: 2, byteEnd: 7}, features: [{$type: ''}]}],
  })
...
describe('RichText#delete w/fat unicode', () => {
  const input = new RichText({
    text: 'one👨‍👩‍👧‍👧 two👨‍👩‍👧‍👧 three👨‍👩‍👧‍👧',
    facets: [{index: {byteStart: 29, byteEnd: 57}, features: [{$type: ''}]}],
  })
...
describe('RichText#segments', () => {
  ...
  it('produces 3 segments with 1 entity in the middle', () => {
    const input = new RichText({
      text: 'one two three',
      facets: [{index: {byteStart: 4, byteEnd: 7}, features: [{$type: ''}]}],
    })
  ...
  it('produces 2 segments with 1 entity in the byteStart', () => {
    const input = new RichText({
      text: 'one two three',
      facets: [{index: {byteStart: 0, byteEnd: 7}, features: [{$type: ''}]}],
    })
    expect(Array.from(input.segments())).toEqual([
      {
        text: 'one two',
  ...
  it('produces 2 segments with 1 entity in the end', () => {
    const input = new RichText({
      text: 'one two three',
      facets: [{index: {byteStart: 4, byteEnd: 13}, features: [{$type: ''}]}],
    })
    expect(Array.from(input.segments())).toEqual([
      {text: 'one '},
      {
  ...
  it('produces 1 segments with 1 entity around the entire string', () => {
    const input = new RichText({
      text: 'one two three',
      facets: [{index: {byteStart: 0, byteEnd: 13}, features: [{$type: ''}]}],
    })
    expect(Array.from(input.segments())).toEqual([
      {
        text: 'one two three',
